...
func ComputeCodexModelsHash(models []config.CodexModel) string {
	keys := normalizeModelPairs(func(out func(key string)) {
		for _, model := range models {
			...
			out(strings.ToLower(name) + "|" + strings.ToLower(alias) + "|" + strings.TrimSpace(model.DisplayName) + "|" + fmt.Sprintf("force-mapping=%t|support-configuration-update=%t", model.ForceMapping, model.SupportConfigurationUpdate))
		}
	})
	return hashJoined(keys)
}
